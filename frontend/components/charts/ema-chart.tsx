...
function EmaChartInner({ data }: { data: IndicatorPoint[] }) {
      ...
      <LineChart data={formatted} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
        ...
        <Tooltip
          ...
          formatter={(value, name) => {
            ...
            if (name === "value") return [Number(value).toFixed(2), "EMA"];
...
export function EmaChart({ data, height = 180 }: EmaChartProps) {
  if (!data || data.length === 0) return null;
  return (
    <ChartContainer height={height}>
      <EmaChartInner data={data} />
    </ChartContainer>
  );
}
